...
SLIDES = [
    ...
    ("准备工作", ["1. 安装 Node.js 20 或更高版本", "2. 在会员页下载 Windows 一键包并完整解压", "3. 电脑时间开启自动同步", "4. 固定公网 IP 不是必需条件，后面可以二选一"],
     "第一步是准备环境。安装 Node.js 二十或更高版本，安装时保持默认选项。登录 MOOX 会员区，进入 AI 交易安全接入页面，下载 Windows 一键包，然后完整解压，不要直接在压缩包里运行。电脑系统时间必须开启自动同步，因为交易所签名依赖准确时间。固定公网 IP 不是接入的必需条件：有固定 IP 的会员推荐绑定白名单；没有固定 IP 的会员可以选择不绑定，但需要承担更高的密钥泄露风险。"),
    ...
    ("在 Bitget 创建 UTA API", ["Bitget 个人中心 → API 管理 → 创建 API", "设置独立 Passphrase 并离线保存", "只开启 UTA 管理/读取 + UTA 交易", "禁止：提币、划转、资金转出、跟单相关权限", "IP 白名单可选；固定公网 IP 用户推荐绑定"],
     "第四步，在 Bitget 个人中心进入 API 管理并创建 API。创建时设置一个独立的 Passphrase，并和 Key、Secret 一起离线保存。权限只开启统一交易账户的管理读取权限，以及统一交易账户的交易权限。任何提币、划转、资金转出或者不需要的跟单权限都不要开启。IP 白名单可以选择：有固定公网 IPv4 的会员建议绑定；没有固定 IP 的会员可以暂不绑定。无论选哪种方式，都绝对不要勾选提现或划转。"),
    ...
    ("选择 IP 白名单策略", ["有固定公网 IP：MOOX_REQUIRE_IP_WHITELIST=true（推荐）", "无固定公网 IP：MOOX_REQUIRE_IP_WHITELIST=false", "关闭白名单不等于开放提现权限", "动态 IP 用户应使用独立 Key、定期轮换、异常立即删除"],
     "接下来选择 IP 白名单策略。如果运行 Agent 的电脑或 VPS 有固定公网出口 IP，建议在 Bitget 绑定白名单，并保持配置为 true。如果使用家庭宽带、移动网络、动态 IP，或者根本没有固定公网 IP，可以把配置改成 false。关闭这个开关只是不再强制检查 IP 白名单，不会放宽止损、仓位和保护单，也绝不能开启提现或划转权限。动态 IP 用户应使用独立 API Key，定期更换，一旦电脑异常就立即在 Bitget 删除 Key。"),
    ...
    ("DRY_RUN 检查", ["双击：2-检查DRY_RUN.bat", "只验证，不下单", "检查权限、所选 IP 策略、UTA 账户设置、精确合约、时间同步", "必须拒绝提现权限、过期计划、错误点位和缺失保护单"],
     "完成配置后双击二号检查 DRY RUN。它会连接 Bitget 做只读验证，但不会下单。重点检查 API 权限、你选择的 IP 白名单策略、统一账户设置、交易品种是否为精确在线合约，以及电脑时间是否同步。如果选择强制白名单但 Bitget 没有绑定 IP，Agent 会停止并告诉你修改。如果选择不强制白名单，Agent 会显示风险提示，但提现权限、过期计划、错误点位、行情过旧和缺少保护条件仍然会被拒绝。"),
...
def build_pdf() -> None:
    ...
    sections = [
        ("一、准备环境", ["安装 Node.js 20 或更高版本。", "在会员页下载 Windows 一键包，完整解压后再运行。", "电脑系统时间开启自动同步。", "固定公网 IPv4 不是必需条件；有固定 IP 推荐绑定，没有固定 IP 可选择关闭白名单检查。"]),
        ...
        ("四、在 Bitget 创建 UTA API", ["Bitget 个人中心 -> API 管理 -> 创建 API。", "设置独立 Passphrase，并离线保存 Key、Secret、Passphrase。", "只开启 UTA 管理/读取与 UTA 交易；禁止提币、划转和不必要权限。", "IP 白名单可选；固定公网 IP 用户推荐绑定。"]),
    ]
    ...
    data = [[Paragraph("来源", small), Paragraph("粘贴位置", small), Paragraph("说明", small)],
            ...
            [Paragraph("IP 白名单策略", body), Paragraph("MOOX_REQUIRE_IP_WHITELIST=", body), Paragraph("固定 IP 填 true；动态 IP 填 false", body)],
    ...
    story += [table, Spacer(1, 5 * mm), Paragraph("示例值必须打码：BITGET_API_KEY=bg_****。不要添加引号，不要把等号左边的名称改掉。", warn), Paragraph("六、选择 IP 白名单策略", h1), Paragraph("有固定公网 IPv4 或固定出口 IP：保持 MOOX_REQUIRE_IP_WHITELIST=true，并在 Bitget 绑定该 IP（推荐）。没有固定公网 IP、使用家庭宽带或移动网络：改为 MOOX_REQUIRE_IP_WHITELIST=false，可以继续接入，但 API Key 泄露风险更高。两种方式都必须关闭 withdraw、transfer、提币和划转权限。", body), Paragraph("关闭白名单只取消 IP 强制检查，不会放宽止损、仓位、回撤、行情新鲜度或保护单。动态 IP 用户应使用独立 API Key、定期轮换，并在设备异常时立即到 Bitget 删除 Key。", warn), PageBreak(), Paragraph("七、六种试运行方法", h1)]
