...
export default async function MemberGannPage() {
  ...
  return <><MemberDeviceHeartbeat /><main><Section spacing="lg"><div className="mx-auto w-full max-w-7xl space-y-8">
    <header className="rounded-3xl border border-amber-300/20 bg-[radial-gradient(circle_at_88%_0%,rgba(251,191,36,.16),transparent_34%),linear-gradient(145deg,#15120b,#090a0e)] p-6 sm:p-8">
      ...
      <Text variant="body" color="secondary" className="mt-3 block max-w-4xl">近6个月严格复盘后，江恩最有价值的是明确时间窗、动态支撑压力和价格目标。当前前瞻研究权重 {forwardSummary.effectiveWeightPct}%：用于提高关键日与点位信心；累计满 {forwardSummary.minimumSamples} 个独立评分样本后才按收缩后可靠度自动调整，且不改写已锁定周方向，也不绕过仓位、止损和保护单门禁。</Text>
    ...
    <section className="rounded-3xl border border-sky-300/15 bg-sky-300/[.035] p-5 sm:p-6">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Card padding="md"><Text variant="caption" color="tertiary">调权样本门槛</Text><p className="mt-2 text-2xl font-semibold">{forwardSummary.scored}/{forwardSummary.minimumSamples}</p></Card><Card padding="md"><Text variant="caption" color="tertiary">收缩后可靠度</Text><p className="mt-2 text-2xl font-semibold">{forwardSummary.adjustedReliabilityPct == null ? "积累中" : `${forwardSummary.adjustedReliabilityPct}%`}</p></Card><Card padding="md"><Text variant="caption" color="tertiary">当前研究权重</Text><p className="mt-2 text-2xl font-semibold">{forwardSummary.effectiveWeightPct}%</p></Card><Card padding="md"><Text variant="caption" color="tertiary">调权状态</Text><p className="mt-2 text-2xl font-semibold">{forwardSummary.eligible ? "已达门槛" : "学习中"}</p></Card></div>
    ...
    ].map(([title, body]) => <div key={title} className="rounded-2xl border border-white/[.08] bg-black/20 p-4"><p className="font-semibold">{title}</p><p className="mt-2 text-body-sm leading-6 text-foreground-secondary">{body}</p></div>)}</div><Text variant="caption" color="tertiary" className="mt-4 block">当前 {forwardSummary.effectiveWeightPct}% 是研究层权重，不是胜率承诺，也不是下单比例。满 {forwardSummary.minimumSamples} 个独立前瞻评分样本后，系统按Beta(2,2)收缩可靠度在0–5点范围自动调整；表现下降会降权，仍不会获得交易权限。</Text></section>
