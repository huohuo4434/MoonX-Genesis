...
export default async function MemberCryptoPicksPage(){noStore();const gate=await getMemberDevicePageAccess();if(gate.status==="LOGIN_REQUIRED")redirect(`/login?next=${path}`);if(gate.status==="MEMBERSHIP_REQUIRED")redirect("/pricing");if(gate.status==="DEVICE_REQUIRED")return <main><Section spacing="lg"><MemberDeviceGate decision={gate.device} nextPath={path}/></Section></main>;const payload=await getConvictionListPagePayload();return <><MemberDeviceHeartbeat/><main className="min-h-screen bg-[#07080a] text-white"><Section spacing="lg"><div className="mx-auto w-full max-w-[1180px]"><Badge variant="default">会员专享</Badge><Heading as="h1" size="h2" className="mt-4 text-white">加密货币推荐</Heading><Text variant="body" className="mt-3 block max-w-3xl text-white/58">每日更新研究截面；方向看当天结论，评级表示研究优先级，不代表必涨。</Text><div className="mt-4 flex flex-wrap gap-2 text-xs text-white/55"><span className="rounded-full border border-white/10 px-3 py-1.5">A+ 核心跟踪</span><span className="rounded-full border border-white/10 px-3 py-1.5">A 重点跟踪</span><span className="rounded-full border border-white/10 px-3 py-1.5">A− 高波动观察</span><span className="rounded-full border border-white/10 px-3 py-1.5">B 实验观察</span><span className="rounded-full border border-emerald-300/20 px-3 py-1.5 text-emerald-100">BTC/ETH 风险中</span><span className="rounded-full border border-amber-300/20 px-3 py-1.5 text-amber-100">SOL/HYPE 风险高</span><span className="rounded-full border border-rose-300/20 px-3 py-1.5 text-rose-100">太空狗 风险极高</span></div><div className="mt-7"><MemberRecommendationList payload={payload} kind="CRYPTO"/></div></div></Section></main></>}
