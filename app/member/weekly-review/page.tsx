...
export default async function MemberWeeklyReviewPage() {
  ...
  return <><MemberDeviceHeartbeat /><main><Section spacing="lg"><div className="mx-auto w-full max-w-6xl space-y-8">
    ...
    <section className="space-y-4"><div><Heading as="h2" size="h3">来源反馈闭环</Heading><Text variant="body-sm" color="secondary" className="mt-1 block">只给“目标周开始前已冻结独立方向／路径”的来源记分。旧记录只有来源编号、没有独立结论的，保留为关联证据但不冒充命中样本。</Text></div><div className="grid gap-3 md:grid-cols-2">{payload.sourcePerformance.map((row) => <Card key={row.sourceKey} padding="md" className="border-border/[0.09] bg-card/40"><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="font-semibold">{row.publicLabel}</p><p className="mt-1 text-caption text-foreground-tertiary">{row.roleLabel}</p></div><Badge variant={row.state === "ELIGIBLE" ? "success" : row.state === "TIMING_ONLY" ? "info" : "outline"}>{row.state === "ELIGIBLE" ? "达到调权门槛" : row.state === "TIMING_ONLY" ? "单独验证择时" : `学习中 ${row.attributableSamples}/${row.minimumSamples}`}</Badge></div><div className="mt-3 flex flex-wrap gap-2"><Badge variant="success">完整 {row.full}</Badge><Badge variant="warning">部分 {row.partial}</Badge><Badge variant="danger">未中 {row.miss}</Badge><Badge variant="neutral">仅关联 {row.linkedOnlySamples}</Badge></div><p className="mt-3 text-body-sm text-foreground-secondary">{row.weightedAccuracyPct == null ? "尚无可独立归因的完整周样本" : `样本加权命中 ${row.weightedAccuracyPct}% · 收缩后可靠度 ${row.adjustedReliabilityPct}%`}{row.confidenceAdjustmentPct ? ` · 后续置信调整 ${row.confidenceAdjustmentPct > 0 ? "+" : ""}${row.confidenceAdjustmentPct}%` : ""}</p><p className="mt-2 text-caption leading-5 text-foreground-tertiary">{row.authorityBoundary}</p></Card>)}</div></section>
