...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return buildLocalizedPageMetadata({
    ...
    basePath: path,
    titleZh: "会员股票研究 | MOOX",
    titleEn: "Member Stock Research | MOOX",
    descriptionZh: "从年度路线、整月阶段、本周六爻到日内奇门与缠论交叉验证。",
    descriptionEn: "Member research from yearly and monthly context to weekly Liu Yao and daily cross-checks.",
  });
}

export default async function MemberStockPicksPage() {
  noStore();
    ...
    <main className="min-h-screen bg-[#07080a] text-white">
      <Section spacing="lg">
        <div className="mx-auto w-full max-w-[1240px]">
          ...
          <Heading as="h1" size="h2" className="mt-4 text-white">股票研究 · 年月周日路线</Heading>
          <Text variant="body" className="mt-2 block max-w-4xl text-white/58">先选标的看最终方向；感兴趣再看年、月、周、日完整依据。</Text>
          <div className="mt-5"><MemberStockResearchDashboard rows={payload.stockResearchRows ?? []} /></div>
          <Link href="/member/sector-resonance" className="mt-6 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-cyan-300/15 bg-cyan-300/[.04] px-5 py-4 text-sm text-cyan-100/75 transition hover:border-cyan-300/30 hover:bg-cyan-300/[.07]">
            <span><b className="text-white">查看板块共振分析</b><span className="ml-2 text-white/42">年度背景、本周与9月各周全部重点资产对照</span></span>
            <span>进入 →</span>
          </Link>
        </div>
      </Section>
