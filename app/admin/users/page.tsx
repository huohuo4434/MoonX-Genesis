...
import { MemberGrowthPanel } from "@/components/admin/MemberGrowthPanel";
import { readMemberGrowth } from "@/lib/analytics/member-growth-server";
...
async function GrowthSnapshot() {
  try { return <MemberGrowthPanel report={await readMemberGrowth()} />; }
  catch { return <MemberGrowthPanel report={null} />; }
}
...
export default async function AdminUsersPage({ searchParams }: { searchParams: Promise<{ q?: string; sandbox?: string }> }) {
  ...
  return <main><Section spacing="lg"><AdminNav current="/admin/users" /><Heading as="h1" size="h2">用户与会员</Heading>
    {!showSandbox ? <Suspense fallback={<p className="my-4" role="status">正在核对经营数据，不影响下方用户列表。</p>}><GrowthSnapshot /></Suspense> : null}
    {!showSandbox ? <details className="mt-4"><summary className="cursor-pointer py-3">注册来源转化核对 · 旧权益口径（不是付费率）</summary><Card padding="md"><Heading as="h2" size="h3">来源与当前有效权益</Heading><div className="mt-3 grid gap-3 sm:grid-cols-2"><div><Text variant="body-sm" weight="semibold">近 7 天</Text><Text variant="caption" color="tertiary" className="mt-1 block">注册 {attribution7d.registrations} · X 来源 {attribution7d.xRegistrations} · X 当前有效会员 {attribution7d.xActiveMembers} · 有效权益占比 {attribution7d.xConversionPercent === null ? "—" : `${attribution7d.xConversionPercent}%`}</Text></div><div><Text variant="body-sm" weight="semibold">近 30 天</Text><Text variant="caption" color="tertiary" className="mt-1 block">注册 {attribution30d.registrations} · X 来源 {attribution30d.xRegistrations} · X 当前有效会员 {attribution30d.xActiveMembers} · 有效权益占比 {attribution30d.xConversionPercent === null ? "—" : `${attribution30d.xConversionPercent}%`}</Text></div></div><Text variant="caption" color="tertiary" className="mt-3 block">有效权益可能包含赠送，不代表已付款。来源追踪从本版本上线后开始；未带活动参数或首次访问早于本版本的用户不会被倒推归因。已追踪：7天 {attribution7d.trackedRegistrations}/{attribution7d.registrations}，30天 {attribution30d.trackedRegistrations}/{attribution30d.registrations}。</Text></Card></details> : null}
