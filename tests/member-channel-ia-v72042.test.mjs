...
assert.equal((nav.match(/groupKey:/g) ?? []).length, 8, "member dropdown includes the existing seven tasks plus personal notes");
assert.match(nav, /memberNotes: "\/member\/notes"/);
...
const memberHub = read("components/member/MemberChannelContent.tsx");
assert.match(read("app/member/page.tsx"), /MemberChannelContent locale=\{locale\} active=\{active\}/);
...
assert.match(memberHub, /localizeHref\(item.href, locale\)/);
...
assert.match(memberHub, /active \? <SeptemberSemiconductorSpotlight locale=\{locale\} \/> : null/);
