...
test("shared period gate permits explicit continuous duration, not a missing fixed deadline", () => {
  const now = new Date("2056-09-06T00:00:00Z");
  const continuous = { ...row, durationMode: "CONTINUOUS", endsAt: null };
  assert.equal(livePeriodReadiness(continuous, now), "READY");
  assert.equal(livePeriodReadiness({ ...continuous, durationMode: undefined }, now), "INVALID");
  assert.equal(livePeriodReadiness({ ...continuous, status: "STOPPED" }, now), "STOPPED");
  assert.equal(livePeriodReadiness({ ...continuous, status: "COMPLETED" }, now), "EXPIRED");
});
