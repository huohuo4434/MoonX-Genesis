...
test("weekly path classification and scoring preserve full partial and miss outcomes", () => {
  ...
  assert.deepEqual(scoreWeeklyVerification("震荡上涨", "先跌后涨"), { result: "PARTIAL_HIT", directionScore: 45, pathScore: 20, totalScore: 65 });
  assert.deepEqual(scoreWeeklyVerification("震荡上涨", "上涨"), { result: "PARTIAL_HIT", directionScore: 45, pathScore: 20, totalScore: 65 });
  assert.deepEqual(scoreWeeklyVerification("震荡下跌", "先涨后跌"), { result: "PARTIAL_HIT", directionScore: 45, pathScore: 20, totalScore: 65 });
  assert.deepEqual(scoreWeeklyVerification("上涨", "先跌后涨"), { result: "PARTIAL_HIT", directionScore: 45, pathScore: 20, totalScore: 65 });
