...
test("navigation has one clear destination per href and keeps specialist tools in the member group", () => {
  ...
  assert.deepEqual(PUBLIC_MORE_NAV.map((item) => item.labelZh), ["新手指南", "方法论", "会员价格", "客服与帮助"]);
...
test("homepage shows the product before pricing and no longer publishes raw plan-ledger positions", () => {
  ...
  assert.ok(page.indexOf("<HomeTodaySection />") < page.indexOf("<HomeMembershipComparison />"));
  assert.ok(page.indexOf("<HomeFeaturedAssets />") < page.indexOf("<HomeMembershipComparison />"));
