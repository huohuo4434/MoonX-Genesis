...
4. 六爻与奇门只能使用已提供笔记，并分别独立给出方向。丙午老师是六爻来源唯一第一优先级；狼叔与用户按老师方法解读的自起六爻同为第二优先级。狼叔与用户冲突时不得默认选择狼叔；只有在发布前，奇门、至少3名独立已批准博主的严格多数、完整缠论结构全部同向支持其中一方时，才建议该方拥有正式方向，否则并列展示、降低信心并要求人工复核。外部三层不得自己定方向，分歧必须保留；关键日默认允许前后一个交易日误差。
