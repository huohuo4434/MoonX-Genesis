...
export function HomeWelcome({ locale, canViewDaily = false }: { locale: Locale; canViewDaily?: boolean }) {
  ...
  return <div data-public-welcome className="mx-auto max-w-6xl space-y-10 px-4 py-8 sm:px-6 sm:py-14">
    ...
    <Link href={href("/free-picks")} className="block rounded-2xl border border-cyan-300/25 bg-cyan-400/[0.04] p-6 hover:border-cyan-300/50"><h2 className="text-xl font-semibold">{en ? "Free picks · One complete setup at a time →" : "免费精选 · 隔几天，认真看一条完整计划 →"}</h2><p className="mt-2 text-sm leading-6 text-white/70">{en ? "Reviewed every three days. Qualifying plans include entry conditions, stops, targets and expiry. No suitable setup means waiting. See the current release status; no guaranteed returns." : "每三天复核，合格才发布，写清参与条件、止盈止损和有效期。没有好机会就等待。查看本期状态，不承诺盈利。"}</p></Link>
