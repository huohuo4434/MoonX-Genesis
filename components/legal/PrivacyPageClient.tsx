    ...
    <main>
      <Section spacing="lg" className="mx-auto max-w-2xl">
        ...
        <div className="mt-8 space-y-8 text-body-sm text-foreground-secondary">
          ...
          <section className="space-y-2"><h2 className="text-body font-semibold text-foreground">{en ? "4. Payment data" : "4. 付款数据"}</h2><p>{en ? "Payments are now verified manually. Send only your registered email, plan, network, amount and TXID to official Telegram support. Telegram is an external service with its own privacy policy. Historical order and transfer records and new manual-operation audit records are retained for billing and support. We never request wallet private keys or seed phrases." : "付款现改为人工核验，请仅向官方电报客服提供注册邮箱、套餐、网络、金额和 TXID。Telegram 是外部服务，适用其自身隐私政策。历史订单及转账记录、新的人工操作审计记录将按账务和客服需要保留。我们不会索取钱包私钥或助记词。"}</p></section>
          ...
          <section className="space-y-2"><h2 className="text-body font-semibold text-foreground">{en ? "9. Contact" : "9. 联系方式"}</h2><p>{en ? "Privacy questions" : "隐私问题"}：{supportEmail}</p><p>{en ? "Last updated: Sep 24, 2026" : "最后更新：2026年9月24日"}</p></section>
        </div>
      </Section>
    </main>
  );
