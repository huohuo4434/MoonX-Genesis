...
export function DailyAccuracyClient({ items, stats }: { items: PublicAccuracyHistoryItem[]; stats: DailyAccuracyStats }) {
  ...
  const statsCards = [
    ...
    [en ? "Weighted accuracy" : "加权命中率", sampleReady ? formatPct(displayStats.weightedHitRate ?? displayStats.hitRate, en) : en ? "Building sample" : "样本积累中"],
    [en ? "Full-path accuracy" : "完整路径命中率", sampleReady ? formatPct(displayStats.pathHitRate, en) : en ? "Building sample" : "样本积累中"],
    [en ? "Direction accuracy" : "方向命中率", sampleReady ? formatPct(displayStats.directionHitRate, en) : en ? "Building sample" : "样本积累中"],
  ];
  ...
  return (
    <div className="mx-auto w-full max-w-[1200px] px-4 sm:px-6 lg:px-8">
      ...
      {!items.length ? <Card padding="lg" className="mb-8"><Text variant="body" weight="semibold">{en ? "Verification samples are building" : "历史验证样本正在积累"}</Text><Text variant="body-sm" color="secondary" className="mt-2">{en ? "A record enters statistics only after its forecast window ends and verified market data is available. Future forecasts and blank data never inflate accuracy." : "只有预测周期结束并取得真实行情后才计入统计；不会用未来预测或空白数据填充命中率。"}</Text></Card> : <>
        ...
        {!sampleReady && displayStats.verifiedCount > 0 ? <Text variant="body-sm" color="tertiary" className="mb-6 block">{en ? `Current verified sample: ${displayStats.verifiedCount}. Stable accuracy rates appear after 30 verified records.` : `当前有效样本为 ${displayStats.verifiedCount} 条；累计满30条后再展示稳定命中率。`}</Text> : null}
        <div className="mb-8"><div className="mb-3"><Heading as="h2" size="h3">{en ? "Verification by consensus stars" : "按共识星级验证"}</Heading><Text variant="body-sm" color="secondary" className="mt-1">{en ? "Stars are locked at publication and describe cross-method agreement, not upside size. Older records are not assigned stars retroactively." : "星级在预测发布时锁定。仅统计带星级的新基准记录；旧记录不会倒推星级。"}</Text></div>{ratedSampleCount ? <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{starBuckets.map((bucket) => <Card key={bucket.stars} padding="md"><Text variant="body" weight="semibold" className="text-primary">{starsText(bucket.stars)}</Text><Text variant="caption" color="tertiary" className="mt-2 block">{en ? "Verified sample" : "有效样本"}</Text><Text variant="body" weight="semibold">{bucket.sampleCount}</Text><Text variant="caption" color="tertiary" className="mt-2 block">{en ? "Weighted accuracy" : "加权命中率"}</Text><Text variant="body-sm" weight="semibold">{bucket.sampleCount >= 10 ? formatPct(bucket.weightedHitRate, en) : en ? "Building sample" : "样本积累中"}</Text></Card>)}</div> : <Card padding="md"><Text variant="body-sm" color="secondary">{en ? "Consensus-star verification begins with the new locked standard." : "星级验证从新基准预测开始累计。"}</Text></Card>}</div>
      </>}
